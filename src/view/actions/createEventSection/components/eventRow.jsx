...
export default function DataSectionRow(remove, variable, index, variables) {
  ...
  return (
    <Flex direction="row" gap="size-200" key={`data${id}`}>
      <View flex>
        <WrappedField
          name={`eventJsonPairs.${index}.key`}
          defaultValue={key}
          aria-label={`Event JSON Key ${index}`}
          width="100%"
        />
      </View>
      <View flex>
        <WrappedField
          name={`eventJsonPairs.${index}.value`}
          defaultValue={value}
          aria-label={`Event JSON Value ${index}`}
          width="100%"
          supportDataElement
        />
      </View>
      <View width="size-450">
        {variables.length > 1 && (
          <ActionButton
            aria-label={`Delete Event JSON Row ${index}`}
            isQuiet
            onPress={() => {
              remove(index);
            }}
