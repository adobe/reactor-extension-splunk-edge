...
export default function DataSectionRow(remove, variable, index, variables) {
  ...
  return (
    <Flex direction="row" gap="size-200" key={`data${id}`}>
      <View flex>
        <WrappedField
          name={`fieldsJsonPairs.${index}.key`}
          defaultValue={key}
          aria-label={`Field JSON Key ${index}`}
          width="100%"
        />
      </View>
      <View flex>
        <WrappedField
          name={`fieldsJsonPairs.${index}.value`}
          defaultValue={value}
          aria-label={`Field JSON Value ${index}`}
          width="100%"
          supportDataElement
        />
      </View>
      <View width="size-450">
        {variables.length > 1 && (
          <ActionButton
            aria-label={`Delete Field JSON Row ${index}`}
            isQuiet
            onPress={() => {
              remove(index);
            }}
